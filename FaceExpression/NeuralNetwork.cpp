...
int NeuralNetwork::LRValidate(Mat& face, int nexpression)
{
	Mat labelvector;
	Mat labelmatrix;
	Mat pcalabelvector;
	...
	int pred = 0;
	float softmax = soft.at<float>(0);
	float softtemp;
	for (int k = 1; k<nexpression; k++)
	{
		softtemp = soft.at<float>(k);
		if (softtemp > softmax)
		{
			softmax = softtemp;
			pred = k;
...
std::pair<cv::Mat, cv::Mat> NeuralNetwork::unpackage(Image* package, bool docreatepca)
{
    ...
    {
		f = *it;		if (f.empty()) continue;
		if (f.channels() > 1) cv::cvtColor(f, g, COLOR_BGR2GRAY);
		else g = f;
		ret = detect.detect(g, face, head);		if (!ret) continue;
		h = g(face);
		cout<<"File "<<++count<<endl;
		labelvector = this->labeledVector(h);
		this->zeroMean(labelvector);
